...
import { AdminHubEvents } from "@typeConfigs/signalr";

export const useAdminHub = () => {
  const { user, isAdmin } = useAuth();
  const connection: HubConnection = createHubConnection(ENDPOINT_URLS.ADMIN_HUB, user?.token!);
  ...
  useEffect(() => {
    ...
    if (connection) {
      connection
        .start()
        .then(() => {
          connection.on(AdminHubEvents.UpdatedUserRoles, (userId: string, updatedAt: Date) =>
            setLastAdminAction(`Updated roles for user with Id '${userId}' at ${updatedAt}`)
          );
